                  ...
                  <div class="modal-dialog modal-lg modal-dialog-scrollable">
                    <div class="modal-content rounded-3 shadow">
                      ...
                      <form action="{{ route('user.update', $u->id) }}" method="POST" style="display:inline;">
                        ...
                        <input type="hidden" name="status" value="{{ $u->status === 'approved' ? 'disabled' : 'approved' }}">
                        
                        <button type="submit" class="btn btn-{{ $u->status === 'approved' ? 'danger' : 'success' }}">
                            {{ $u->status === 'approved' ? 'Nonaktifkan' : 'Aktifkan' }}
                        </button>
                    </form>
