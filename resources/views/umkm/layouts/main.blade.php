  ...
  <header id="header" class="header d-flex align-items-center">
    <div class="container-fluid container-xl d-flex align-items-center justify-content-between">
      ...
      <nav id="navbar" class="navbar">
        <ul>
          <li><a href="/umkm" class="nav link {{ ($title === "beranda") ? 'active' : '' }}" >Beranda</a></li>
          <li><a href="/umkmproduk" class="nav link {{ ($title === 'produk' || $title === 'detail') ? 'active' : '' }}">Produk Terbaru</a></li>
          <li class="dropdown "><a href="#" class="nav link {{ $title === "kuliner" ? 'active' : '' }}"><span>Kategori</span> <i class="bi bi-chevron-down dropdown-indicator"></i></a>
            <ul>
              @foreach($SektorUsaha as $p)
                  <li>
                      <a href="{{ route('umkm.produk.sektor', ['sektor' => urlencode($p->nama)]) }}" class="nav link {{ ($title === ucfirst($p->nama)) ? 'active' : '' }}">{{ ucfirst($p->nama) }}</a>
                  </li>
              @endforeach
          </ul>
          ...
          <li><a href="/umkmkontak"class="nav link {{ ($title === "kontak") ? 'active' : '' }}">Kontak</a></li>
          <li><a href="/umkmbantuan" class="nav link {{ ($title === "bantuan") ? 'active' : '' }}">Bantuan</a></li>
          <li class="dropdown profile-menu">
            ...
            <ul>
              @if (Auth::check())
              <li><a class="dropdown-item bi bi-person me-1" href="{{ route('umkm.profil',Auth::user()->umkm->id) }}">Profil saya</a></li>
              <li>
                <form method="post" action="{{ route(name: 'logout') }}" class="dropdown-item p-0 m-0">
                  @csrf
