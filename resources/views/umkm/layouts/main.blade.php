...
<body>
  ...
  <ul>
    <li>
      <a href="/umkm" class="nav-link {{ $title === 'beranda' ? 'active' : '' }}">
        Beranda
      </a>
    </li>
    <li>
      <a href="/umkmproduk" class="nav-link {{ ($title === 'produk' || $title === 'detail') ? 'active' : '' }}">
        Produk
      </a>
    </li>
    <li>
      <a href="{{ route('daftarumkm.index') }}" class="nav-link {{ $title === 'umkm' ? 'active' : '' }}">
        UMKM
      </a>
    </li>
    <li>
      <a href="{{ route('umkmaduan.index') }}" class="nav-link {{ $title === 'aduan' ? 'active' : '' }}">
        Aduan
      </a>
    </li>
    ...
    <li class="dropdown profile-menu">
      <a href="#" class="nav-link dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false">
        <img src="{{ Auth::check() && Auth::user()->umkm && Auth::user()->umkm->logo 
                    ? asset('storage/' . Auth::user()->umkm->logo)
  ...
  @if (Auth::check())
    @if (Auth::user()->umkm)
      <li>
        <a class="dropdown-item d-flex align-items-center gap-2 py-2" href="{{ route('umkmprofil.index') }}">
          <i class="bi bi-person fs-5"></i>
          <span class="flex-grow-1">Profil Saya</span>
        </a>
      </li>
      <li>
        <a class="dropdown-item d-flex align-items-center gap-2 py-2" href="{{ route('produksaya.index') }}">
          <i class="bi bi-box fs-5"></i>
          <span class="flex-grow-1">Produk Saya</span>
        </a>
      </li>
      <li>
        <a class="dropdown-item d-flex align-items-center gap-2 py-2" href="{{ route('umkmpesanan.index') }}">
          <i class="bi bi-cart fs-5"></i>
          <span class="flex-grow-1">Pesanan</span>
        </a>
      </li>
      <li>
        <a class="dropdown-item d-flex align-items-center gap-2 py-2" href="{{ route('umkmriwayat.index') }}">
